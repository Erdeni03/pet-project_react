...
const PaintCanvas = () => {
    ...
    const {setCanvas, setTool, pushUndo, setCtx, setUsername, setSocket, setSessionId} = useActions()
    ...
    return (
        <>
            <CustomModal open={open} label={'Введите имя пользователя'} setOpen={setOpen} actionBtn={connectHandler}>
                <TextField
                    autoFocus
                    margin="dense"
                    id="name"
        ...
        <div className={classes.canvas}>
            <canvas onMouseDown={() => mouseDownHandler()} ref={canvasRef} className={classes.inlineCanvas} width={1000}
                    height={400}/>
        </div>
        </>
    );
